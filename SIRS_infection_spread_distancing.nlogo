...
"" ""
PENS
"% Infected (with SD)" 1.0 0 -13345367 true "" "if (SD? = True) [plotxy ticks ((count (infecteds))*(100)/(count(turtles)))]"
"% Infected (no SD)" 1.0 0 -2674135 true "" "if (SD? = False) [plotxy ticks ((count (infecteds))*(100)/(count(turtles)))]"
...
Additionally, effective testing (either large number of individuals being tested at each step, or more tests being conducted in the same amount of time) can also flatten the curve.
...
Suppose quarantined individuals aren't permanently removed but are instead 're-inserted' into the population has healthy individuals once the infection period has passed. How does this affect dynamics?
Suppose individuals in the population can reproduce and die of old age (or die due to the disease itself). How is the model affected by this?
